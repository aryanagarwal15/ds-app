...
export const useWebRTC = (
  ...
  const handleRealtimeMessage = useCallback((message: DataChannelMessage) => {
  ...
  }, [setConnectionState, setTranscript, setResponse, setError, startRippleAnimation, stopRippleAnimation]);

  // Setup data channel event handlers
  const setupDataChannel = useCallback((dataChannel: any) => {
    dataChannel.onopen = () => {
      ...
      (peerConnection as any).ontrack = (event: any) => {
        try {
          ...
          if (event.track.kind === 'audio') {
            console.log('Remote audio track received - audio will play automatically');
          }
        } catch (err) {
